...
const PUBLIC_CONTRACT_ARTIFACT = /^coding-tooling-public-contract-v(\d+)-d(\d+)-vfy(\d+)-u(\d+)-i(\d+)-f(\d+)-(\d+)-(\d+)$/;
...
export function derivePathSignals(paths) {
  ...
  const publicContractWorkflowPath = workflows.find((path) => /public[-_.]?contract/.test(workflowBasename(path))) ?? null;
...
export function parsePublicContractArtifactName(name) {
  const match = PUBLIC_CONTRACT_ARTIFACT.exec(String(name ?? ""));
  if (!match) return null;
  const [, schemaVersion, discovered, verified, unverified, incomplete, failedEvidence, runId, attempt] = match;
  const parsed = {
    ...
    failedEvidence: Number(failedEvidence),
    runId: Number(runId),
    attempt: Number(attempt),
  };
  if (!Object.values(parsed).every(Number.isSafeInteger)) return null;
  if (parsed.discovered !== parsed.verified + parsed.unverified) return null;
  if (parsed.verified > parsed.discovered || parsed.incomplete > parsed.discovered) return null;
  return {
    ...parsed,
    verifiedRatio: parsed.discovered === 0 ? 1 : parsed.verified / parsed.discovered,
    strictReady: parsed.unverified === 0 && parsed.incomplete === 0 && parsed.failedEvidence === 0,
  };
}

function publicContractFromRun(owner, repo, configured, runsResult, artifactsResult) {
  if (!configured) return { state: "not-configured" };
  if (runsResult === null) return { state: "unavailable" };
  const run = runsResult?.workflow_runs?.[0] ?? null;
  if (!run || artifactsResult === null) return { state: "unavailable" };
  const candidates = (artifactsResult?.artifacts ?? [])
    .filter((artifact) => artifact.expired !== true)
    .map((artifact) => ({ artifact, summary: parsePublicContractArtifactName(artifact.name) }))
    .filter((candidate) => candidate.summary !== null && candidate.summary.runId === run.id)
    .sort((left, right) => String(right.artifact.created_at ?? "").localeCompare(String(left.artifact.created_at ?? "")));
  const latest = candidates[0];
  if (!latest) return { state: "unavailable" };
  return {
    state: "measured",
    ...latest.summary,
    artifactName: latest.artifact.name,
    createdAt: latest.artifact.created_at ?? null,
    runUrl: run.html_url ?? `https://github.com/${owner}/${repo}/actions/runs/${run.id}`,
  };
}
...
function compactArtifacts(artifacts) {
  return (artifacts?.artifacts ?? [])
    .filter((artifact) => /benchmark|perf|profile|moonlight|coverage|lcov|public-contract/i.test(artifact.name))
    .slice(0, 12)
...
async function collectRepository(client, owner, repo, staleAfterDays) {
  ...
  let publicContractRuns = null;
  let publicContractArtifacts = null;
  if (pathSignals.publicContractWorkflowPath) {
    publicContractRuns = await client.latestWorkflowRun(
      owner,
      repo.name,
      branch,
      pathSignals.publicContractWorkflowPath.split("/").at(-1),
    );
    const publicContractRun = publicContractRuns?.workflow_runs?.[0] ?? null;
    if (publicContractRun) {
      publicContractArtifacts = await client.workflowArtifacts(owner, repo.name, publicContractRun.id);
    }
  }
  const publicContract = publicContractFromRun(
    owner,
    repo.name,
    pathSignals.publicContractWorkflowPath !== null,
    publicContractRuns,
  ...
  return {
    ...
    collection: {
      ...
      publicContractActionsAvailable: pathSignals.publicContractWorkflowPath
        ? publicContractRuns !== null && publicContractArtifacts !== null
        : null,
    },
  };
}
    ...
    while (true) {
      ...
      try {
        ...
        results[index] = {
          ...
          publicContract: { state: "unknown" },
          pipeline: { state: "unknown", workflow: null, conclusion: null, status: null, url: null, updatedAt: null, failureJobs: [] },
          evidenceArtifacts: [],
          metrics: [],
          collection: { treeAvailable: false, actionsAvailable: false, publicContractActionsAvailable: false, error: error instanceof Error ? error.message : String(error) },
        };
      }
    }
  }
