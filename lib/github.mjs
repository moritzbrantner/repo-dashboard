...
export class GitHubClient {
  ...
  repositoryArtifacts(owner, repo) {
    return this.request(`/repos/${owner}/${repo}/actions/artifacts?per_page=100`, { optional: true });
  }
...
function publicContractFromArtifacts(owner, repo, configured, artifactsResult) {
  if (!configured) return { state: "not-configured" };
  if (artifactsResult === null) return { state: "unavailable" };
  ...
  return {
    ...
    runUrl: `https://github.com/${owner}/${repo}/actions/runs/${latest.summary.runId}`,
  };
}
...
async function collectRepository(client, owner, repo, staleAfterDays) {
  ...
  if (pathSignals.publicContractWorkflowPath) {
    publicContractArtifacts = await client.repositoryArtifacts(owner, repo.name);
  }
  const publicContract = publicContractFromArtifacts(
    owner,
    repo.name,
  ...
  return {
    ...
    collection: {
      ...
      publicContractArtifactsAvailable: pathSignals.publicContractWorkflowPath ? publicContractArtifacts !== null : null,
    },
  };
}
    ...
    while (true) {
      ...
      try {
        ...
        results[index] = {
          ...
          evidenceArtifacts: [],
          metrics: [],
          collection: { treeAvailable: false, actionsAvailable: false, publicContractArtifactsAvailable: false, error: error instanceof Error ? error.message : String(error) },
        };
      }
    }
  }
