...
async function enrichRepository(client, config, repo, registry) {
  ...
  let dependencyArchitecture = {
    state: metadata ? "not-declared" : "unregistered",
    repositoryLayer: null,
    dependencies: [],
  };
  ...
  if (!metadata && existingPriority >= 30) {
    action = classificationAction(repo);
  } else if (
    metadata &&
    ["invalid", "unavailable"].includes(dependencyArchitecture.state) &&
    existingPriority >= 24
  ) {
...
function summarizeLandscape(repositories, graph) {
  ...
  const contractStates = ["declared", "not-declared", "invalid", "unavailable", "unregistered"];
  return {
    registered,
