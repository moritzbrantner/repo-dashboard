...
export function summarizeFleet(repositories) {
  ...
  const contractDiscovered = measuredContracts.reduce((sum, repo) => sum + (repo.publicContract?.discovered ?? 0), 0);
  const contractVerified = measuredContracts.reduce((sum, repo) => sum + (repo.publicContract?.verified ?? 0), 0);
  const contractUnverified = measuredContracts.reduce((sum, repo) => sum + (repo.publicContract?.unverified ?? 0), 0);
  const contractIncomplete = measuredContracts.reduce((sum, repo) => sum + (repo.publicContract?.incomplete ?? 0), 0);
  return {
    total,
    passing,
    failing,
...
export function validateMetric(metric) {
  ...
  if (!["benchmark", "coverage", "performance", "quality"].includes(metric.source)) return false;
  return true;
}
